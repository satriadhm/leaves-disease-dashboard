...
class App {
	...
	setupRouter() {
		this.router.addRoute("/", LandingPage, {
			title: "PlantAI - AI-Powered Plant Disease Detection",
			description:
		...
		this.router.addRoute("/home", HomePage, {
			...
			description:
				"Deteksi penyakit tanaman dengan AI yang akurat dan mudah digunakan",
		});
		...
		this.router.addRoute("/register", RegisterPage, {
			...
			description:
				"Buat akun baru untuk mengakses fitur deteksi penyakit tanaman",
		});

		// this.router.addRoute("/landing", LandingPage, {
		// 	title: "PlantAI - AI-Powered Plant Disease Detection",
		// 	description:
		// 		"Tingkatkan hasil panen hingga 40% dengan teknologi AI terdepan. Deteksi penyakit tanaman dalam hitungan detik dengan akurasi 95%+",
		// });
		...
		this.router.addRoute("/pitch", PitchDeckPage, {
			...
			description:
				"Investment presentation showcasing our AI-powered agricultural solution",
		});
		...
		this.router.setErrorHandler((error, path) => {
			...
			window.dispatchEvent(
				new CustomEvent("showError", {
					detail:
						"Terjadi kesalahan saat memuat halaman. Silakan refresh atau coba lagi.",
				})
			);
		});
	}
        ...
        `;
			...
			afterRender() {
				...
				pitchLink?.addEventListener("click", (e) => {
					e.preventDefault();
					window.dispatchEvent(
						new CustomEvent("navigate", { detail: "/pitch" })
					);
				});
			}
		};
		...
		if (storedVersion && storedVersion !== currentVersion) {
			...
			setTimeout(() => {
				window.dispatchEvent(
					new CustomEvent("showInfo", {
						detail:
							"🆕 Aplikasi telah diperbarui dengan fitur baru termasuk pitch deck!",
					})
				);
			}, 2000);
